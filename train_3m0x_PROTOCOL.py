...
if __name__ == "__main__":
    ...
    import os as _os_transfer
    from pathlib import Path as _Path_transfer

    _init_ckpt = _os_transfer.environ.get("INIT_CKPT_FOR_TRANSFER", "")
    if _init_ckpt and _Path_transfer(_init_ckpt).is_dir():
        print(f"3M0X: transfer-loading compatible weights from {_init_ckpt}")

        def _get_module(_name):
            if _name in hparams:
                return hparams[_name]
            if "modules" in hparams and _name in hparams["modules"]:
                return hparams["modules"][_name]
            return None

        for _module_name in ["wav2vec2", "embedding_model", "mean_var_norm_emb"]:
            _module = _get_module(_module_name)
            if _module is None:
                continue

            _ckpt_file = _Path_transfer(_init_ckpt) / f"{_module_name}.ckpt"
            if not _ckpt_file.exists():
                continue

            try:
                _state = torch.load(_ckpt_file, map_location="cpu")
                if isinstance(_state, dict) and "model" in _state:
                    _state = _state["model"]

                _own = _module.state_dict()
                _filtered = {}
                for _k, _v in _state.items():
                    if _k in _own and tuple(_own[_k].shape) == tuple(_v.shape):
                        _filtered[_k] = _v

                _module.load_state_dict(_filtered, strict=False)
                print(f"3M0X: loaded {_module_name}, matched={len(_filtered)}")
            except Exception as e:
                print(f"3M0X: skipped {_module_name}: {e}")


    print('3M0X: no verification download during training.')

    # Dataset prep (parsing VoxCeleb and annotation into csv files)
    from voxceleb_prepare import prepare_voxceleb  # noqa

    print('3M0X: using protocol CSVs; skipping prepare_voxceleb.')
    print("3M0X: skipping noise preparation.")
    print("3M0X: skipping RIR preparation.")
