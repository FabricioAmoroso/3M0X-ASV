...
if __name__ == "__main__":
    ...
    embeddingDir = os.path.join(params["pretrain_path"], "embeddings")
    enrolEmbDir = os.path.join(embeddingDir, "enrol")
    testEmbDir = os.path.join(embeddingDir, "test")
    ...
    if "subset" in params:
        ...
        enrolEmbFileBase, testEmbFileBase = [f"{affect}.pt" for affect in affects]

    enrolEmbFile = os.path.join(enrolEmbDir, enrolEmbFileBase)
    testEmbFile = os.path.join(testEmbDir, testEmbFileBase)

    if not os.path.exists(enrolEmbFile):
        enrol_dict = compute_embedding_loop(enrol_dataloader)
        torch.save(enrol_dict, enrolEmbFile)
    else:
        logger.info("Enroll Embeddings Pre-Computed: Loading...")
        enrol_dict = torch.load(enrolEmbFile)

    if not os.path.exists(testEmbFile):
        test_dict = compute_embedding_loop(test_dataloader)
        torch.save(test_dict, testEmbFile)
    else:
        logger.info("Test Embeddings Pre-Computed: Loading...")
        test_dict = torch.load(testEmbFile)
