...
sb.core.create_experiment_directory = create_experiment_directory
...
if __name__ == "__main__":
    ...
    xv_file = os.path.join(
        params["pretrain_path"], "stats/train/voxCeleb.pkl"
    )
    ...
    stats_dir = os.path.join(params["pretrain_path"], "stats")
    enrol_stats_dir = os.path.join(stats_dir, "enrol")
    test_stats_dir = os.path.join(stats_dir, "test")
    ndx_dir = os.path.join(stats_dir, "ndx")

    os.makedirs(enrol_stats_dir, exist_ok=True)
    os.makedirs(test_stats_dir, exist_ok=True)
    os.makedirs(ndx_dir, exist_ok=True)

    enrol_stat_file_base = test_stat_file_base = ndx_file_base = "voxCeleb.pkl" # cetuc.pkl
    if "subset" in params:
        affects = params["subset"].split("-")
        affects[0] += affects[1][0]
        if affects[1] in ["Emotional", "All"]:
            affects[0] += affects[1][1:3]
        enrol_stat_file_base, test_stat_file_base = [f"{affect}.pkl" for affect in affects]
        ndx_file_base = test_stat_file_base

    enrol_stat_file = os.path.join(enrol_stats_dir, enrol_stat_file_base)
    test_stat_file = os.path.join(test_stats_dir, test_stat_file_base)
    ndx_file = os.path.join(ndx_dir, ndx_file_base)
    ...
    # print("Example modelset:")
    # print(scores_plda.modelset[:10])

    # print("Example segset:")
    # print(scores_plda.segset[:10])
